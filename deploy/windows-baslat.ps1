...
# Only ever one bot per machine. This script also runs from the Startup
# folder at logon (23 Sep 2026), so a second copy could otherwise start
# beside a running one -- two processes on one Binance account would each
# open their own position for the same signal. The check asks the running
# bot's own /health endpoint rather than just looking at port 10000: a
# just-killed process can still hold the port for a few seconds, and on 23
# Sep 2026 that made this script refuse to start when nothing was running.
try {
    $alive = Invoke-RestMethod -TimeoutSec 3 "http://127.0.0.1:10000/health"
} catch {
    $alive = $null
}
if ($alive) {
    Write-Host "Bot zaten calisiyor. Bu pencere kapanabilir." -ForegroundColor Yellow
    Write-Host "Durum: http://127.0.0.1:10000/status"
